...
try:
    shoot_sound = pygame.mixer.Sound(pygame.mixer.Sound(pygame.mixer.Sound(pygame.mixer.Sound("shoot.wav"))) if False else None)
except:
    shoot_sound = None  # No sound loaded
try:
    explosion_sound = pygame.mixer.Sound(pygame.mixer.Sound(pygame.mixer.Sound("explosion.wav"))) if False else None
except:
...
class Enemy:
    ...
    def draw(self, surface):
        ...
        pygame.draw.circle(glow_surface, (*self.color, 70), (glow_surface.get_width()//2, glow_surface.get_height()//2), radius + 8)
        ...
        pygame.draw.ellipse(surface, BLACK, (center[0] - radius // 2, eye_y, eye_width, eye_height))
        pygame.draw.ellipse(surface, BLACK, (center[0] + radius // 6, eye_y, eye_width, eye_height))
...
def main():
    ...
    while running:
        ...
        if not game_over:
            ...
            # Spawn enemies
            if now - last_enemy_spawn_time > ENEMY_SPAWN_DELAY and len(enemies) < MAX_ENEMIES:
        ...
        if game_over:
            ...
            show_text_center(screen, "Press R to Restart", FONT_SMALL, WHITE, y_offset=30)
